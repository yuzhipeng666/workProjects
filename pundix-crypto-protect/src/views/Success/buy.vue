<template>
  <success-slot class="success-buy">
    ...
    <div slot="other">
      <div class="issues">
        <strong class="title">{{ $t("success.buy.issues.title") }}</strong>
        <i18n
          v-for="(way, index) in wayExercise"
          :key="index"
          class="issue"
          tag="div"
          :path="`success.buy.issues.issue.${way}`"
        >
          <template #earnUnit>
            {{ query.earnUnit }}
          </template>
          ...
          <template #exerciseStartDt>
            {{ query.exerciseStartDt }}
          </template>

          <template #exerciseEndDt>
            {{ query.exerciseEndDt }}
          </template>
        </i18n>
      </div>
      <router-link replace :to="{ name: 'assets' }" tag="button" class="link">{{
        $t("success.button.checkMyAssets")
      }}</router-link>
      <router-link replace :to="{ name: 'plans' }" tag="button" class="link">{{
        $t("success.button.checkMyPlans")
      }}</router-link>
      <router-link replace :to="{ name: 'home' }" tag="button" class="link">{{
        $t("success.button.buyAgain")
      }}</router-link>
    </div>
  </success-slot>
</template>
